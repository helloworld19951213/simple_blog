...
def mysql_opera(sql, flag):
    connection = MySQLdb.connect(host='127.0.0.1', user='root', passwd='im7h322000', db='blog', charset='utf8')
...
def run():
    ...
    sql_mem = "insert into moniter_memory(minion_id,memory_total,memory_used,memory_free," + \
              "memory_share,memory_bu_ca,memory_available,memory_time) values('%s',%s,%s,%s,%s,%s,%s,'%s')" % (
                  minion_id, memory_total, memory_used, memory_free, memory_share, memory_bu_ca, memory_available,
                  memory_time)
